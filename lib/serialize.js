...
module.exports = function serialize(sock, m) {
  ...
  // ===== FAKE TROLI (Order Message) =====
  m.replyFakeTroli = (text, opts = {}) => {
    const orderMsg = {
      orderId: opts.orderId || '1234567890',
      thumbnail: opts.thumbnail || null,
      itemCount: opts.itemCount || 99,
      status: 'INQUIRY',
      surface: 'CATALOG',
      message: text || '',
      orderTitle: opts.title || global.botName || 'Ryuuzaa MD',
      sellerJid: opts.sellerJid || (sock.user?.id || ''),
      token: opts.token || 'RYUUZAA',
      totalAmount1000: opts.amount || 999999,
  ...
  // ===== EXTERNAL AD REPLY (Fake link preview / thumbnail) =====
  m.replyAdReply = (text, opts = {}) => {
    const contextInfo = {
      externalAdReply: {
        title: opts.title || global.botName || 'Ryuuzaa MD',
        body: opts.body || global.ownerName || '',
        mediaType: opts.mediaType || 1, // 1=image, 2=video
        thumbnailUrl: opts.thumbnailUrl || opts.thumbnail || 'https://files.catbox.moe/pv60r7.jpg',
        sourceUrl: opts.sourceUrl || opts.url || 'https://github.com/Pirrzaaaaa',
        renderLargerThumbnail: opts.largeThumb !== false,
        showAdAttribution: opts.showAd || false,
        previewType: opts.previewType || 'PHOTO'
      }
    };
    return sock.sendMessage(m.chat, {
      text: String(text),
  ...
  // ===== INTERACTIVE MESSAGE (Buttons + Sections native) =====
  m.replyInteractive = (text, opts = {}) => {
    const interactiveMsg = {
      body: { text: String(text) },
      footer: { text: opts.footer || global.botName || '' },
      header: opts.header ? {
        title: opts.header.title || '',
        subtitle: opts.header.subtitle || '',
        hasMediaAttachment: !!opts.header.image,
        ...(opts.header.image ? { imageMessage: opts.header.image } : {})
      } : undefined,
      nativeFlowMessage: opts.flow ? {
        buttons: (opts.flow.buttons || []).map(b => ({
          name: b.name || 'quick_reply',
          buttonParamsJson: JSON.stringify(b.params || { display_text: b.text || 'Button', id: b.id || '' })
        }))
      } : undefined
    };
  ...
  // ===== LIMITED OFFER (Countdown/expiry fake) =====
  m.replyLimitedOffer = (text, opts = {}) => {
    const contextInfo = {
      externalAdReply: {
        title: opts.title || '🔥 LIMITED OFFER',
        body: opts.body || 'Penawaran terbatas!',
        mediaType: 1,
        thumbnailUrl: opts.thumbnailUrl || 'https://files.catbox.moe/pv60r7.jpg',
        sourceUrl: opts.sourceUrl || '',
        renderLargerThumbnail: true,
        showAdAttribution: true,
        previewType: 'PHOTO'
      },
      expiration: opts.expiration || Math.floor(Date.now() / 1000) + (opts.expiresIn || 3600),
      expiryTimestamp: opts.expiration || Math.floor(Date.now() / 1000) + (opts.expiresIn || 3600)
    };
    return sock.sendMessage(m.chat, {
      text: String(text),
      contextInfo
  ...
  // ===== NEWSLETTER / CHANNEL REPLY =====
  m.replyNewsletter = (text, opts = {}) => {
    const contextInfo = {
      isForwarded: true,
      forwardedNewsletterMessageInfo: {
        newsletterJid: opts.newsletterJid || '120363XXX@newsletter',
        newsletterName: opts.newsletterName || global.botName || 'Ryuuzaa MD',
        serverMessageId: opts.serverId || Math.floor(Math.random() * 999999)
      },
      ...(opts.externalAdReply ? {
        externalAdReply: {
          title: opts.externalAdReply.title || global.botName || 'Ryuuzaa MD',
          body: opts.externalAdReply.body || '',
          mediaType: 1,
          thumbnailUrl: opts.externalAdReply.thumbnailUrl || 'https://files.catbox.moe/pv60r7.jpg',
          sourceUrl: opts.externalAdReply.sourceUrl || '',
          renderLargerThumbnail: true
        }
      } : {})
    };
  ...
  // ===== IMAGE AS DOCUMENT (kirim gambar sebagai file/document) =====
  m.replyImgDoc = (buffer, opts = {}) => {
    const fileName = opts.fileName || opts.filename || `${global.botName || 'Ryuuzaa_MD'}.jpg`;
    const caption = opts.caption || '';
    const contextInfo = opts.contextInfo || {};

    // Jika mau pakai externalAdReply juga
    if (opts.adReply) {
      contextInfo.externalAdReply = {
        title: opts.adReply.title || global.botName || 'Ryuuzaa MD',
        body: opts.adReply.body || '',
        mediaType: 1,
        thumbnailUrl: opts.adReply.thumbnailUrl || 'https://files.catbox.moe/pv60r7.jpg',
        sourceUrl: opts.adReply.sourceUrl || '',
        renderLargerThumbnail: true
      };
    }
    ...
    return sock.sendMessage(m.chat, {
      ...
      caption,
      contextInfo,
      jpegThumbnail: opts.thumbnail || buffer.slice(0, 1024) // thumbnail kecil
    }, { quoted: m });
  };

  // ===== COMBINED: Text + ExternalAdReply + Newsletter =====
  m.replyFancy = (text, opts = {}) => {
    const contextInfo = {
      ...(opts.newsletter ? {
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
          newsletterJid: opts.newsletter.jid || '120363XXX@newsletter',
          newsletterName: opts.newsletter.name || global.botName || 'Ryuuzaa MD',
